...
class RecommendationService:
    ...
    def _load_runtime_data(self) -> RuntimeData:
        ...
        movie_lookup = movies_df.set_index("movie_id").to_dict(orient="index") if not movies_df.empty else {}
        genre_map = {
        ...
        content_similarity_matrix, vectorizer, movie_index_map = self._build_content_profiles(movies_df)
    ...
    def _build_cache_key(self) -> str:
        movie_stats = Movie.objects.filter(is_active=True).aggregate(count=Count("id"), updated=Max("updated_at"))
        rating_stats = Rating.objects.aggregate(count=Count("id"), updated=Max("updated_at"))
    ...
    def _build_user_item_matrix(self, ratings_df: pd.DataFrame, movies_df: pd.DataFrame) -> pd.DataFrame:
        ...
        pivot = ratings_df.pivot_table(index="user_id", columns="movie_id", values="rating", aggfunc="mean")
    ...
    def _build_item_similarity(self, user_item_matrix: pd.DataFrame) -> pd.DataFrame:
        if user_item_matrix.empty or user_item_matrix.shape[1] < 2:
            return pd.DataFrame(index=user_item_matrix.columns, columns=user_item_matrix.columns).fillna(0.0)

        movie_user_matrix = user_item_matrix.fillna(0.0).T
        similarity = cosine_similarity(movie_user_matrix)
        return pd.DataFrame(similarity, index=movie_user_matrix.index, columns=movie_user_matrix.index)
    ...
    def _build_content_profiles(self, movies_df: pd.DataFrame):
        ...
        movie_index_map = {int(mid): idx for idx, mid in enumerate(movies_df["movie_id"].tolist())}
    ...
    def _build_svd_predictions(self, user_item_matrix: pd.DataFrame):
        ...
        reconstructed_df = pd.DataFrame(reconstructed, index=centered.index, columns=centered.columns)
        prediction_df = reconstructed_df.add(user_means, axis=0)
        user_index_map = {int(uid): idx for idx, uid in enumerate(prediction_df.index.tolist())}
    ...
    def recommend_for_user(
        ...
        resolved_model = self._resolve_model(user=user, requested_model=model_key, scenario=scenario)
        recommendations = self._generate_by_model(user=user, model_key=resolved_model, top_k=top_k, scenario=scenario)
        return {
            "requested_model": model_key,
            "resolved_model": resolved_model,
            "resolved_model_label": MODEL_LABELS.get(resolved_model, resolved_model.title()),
            "scenario": scenario,
            "scenario_label": "New user cold start" if scenario == SCENARIO_NEW_USER else "Normal",
    ...
    def _generate_by_model(self, user: User, model_key: str, top_k: int, scenario: str) -> list[dict[str, Any]]:
        ...
        if model_key == MODEL_POPULARITY:
            ...
            return self._materialize_items(ranked, model_key=MODEL_POPULARITY, user=user, scenario=scenario)
        if model_key == MODEL_CONTENT:
            ranked = self._merge_with_fallback(content_df, popularity_df).head(top_k)
            return self._materialize_items(ranked, model_key=MODEL_CONTENT, user=user, scenario=scenario)
        if model_key == MODEL_ITEM:
            ranked = self._merge_with_fallback(item_df, popularity_df).head(top_k)
            return self._materialize_items(ranked, model_key=MODEL_ITEM, user=user, scenario=scenario)
        if model_key == MODEL_SVD:
            ranked = self._merge_with_fallback(svd_df, popularity_df).head(top_k)
            return self._materialize_items(ranked, model_key=MODEL_SVD, user=user, scenario=scenario)
        ...
        return self._materialize_items(ranked, model_key=MODEL_HYBRID, user=user, scenario=scenario)

    def _merge_with_fallback(self, primary_df: pd.DataFrame, fallback_df: pd.DataFrame) -> pd.DataFrame:
        if primary_df.empty:
        ...
        merged["final_score"] = merged[main_col] + (0.05 * merged.get("popularity_norm", 0.0))
        return merged.sort_values(["final_score", "rating_count", "avg_rating"], ascending=False).reset_index(drop=True)
    ...
    def _get_seen_movie_ids(self, user: User, scenario: str) -> set[int]:
        if scenario == SCENARIO_NEW_USER or self.runtime.ratings_df.empty:
            return set()
        user_rows = self.runtime.ratings_df[self.runtime.ratings_df["user_id"] == user.id]
        return set(user_rows["movie_id"].astype(int).tolist())

    def _normalize_series(self, df: pd.DataFrame, column: str, output_column: str) -> pd.DataFrame:
    ...
    def _base_candidate_df(self, user: User, scenario: str) -> pd.DataFrame:
        candidates = self.runtime.movies_df[["movie_id", "avg_rating", "rating_count", "popularity_score"]].copy()
        seen_ids = self._get_seen_movie_ids(user, scenario)
        if seen_ids:
            candidates = candidates[~candidates["movie_id"].isin(seen_ids)].copy()
        return candidates.reset_index(drop=True)

    def _popularity_scores(self, user_id: int, top_k: int | None = None) -> pd.DataFrame:
        candidates = self.runtime.movies_df[["movie_id", "avg_rating", "rating_count", "popularity_score"]].copy()
        seen_ids = set()
        if not self.runtime.ratings_df.empty:
            seen_ids = set(self.runtime.ratings_df[self.runtime.ratings_df["user_id"] == user_id]["movie_id"].astype(int).tolist())
        if seen_ids:
            candidates = candidates[~candidates["movie_id"].isin(seen_ids)].copy()
        candidates = self._normalize_series(candidates, "popularity_score", "popularity_norm")
        ranked = candidates.sort_values(["popularity_norm", "rating_count", "avg_rating"], ascending=False).reset_index(drop=True)
    ...
    def _content_scores(self, user: User, scenario: str) -> pd.DataFrame:
        ...
        if candidates.empty or self.runtime.content_similarity_matrix is None:
            candidates["content_score"] = 0.0
            return candidates

        user_ratings = self.runtime.ratings_df[self.runtime.ratings_df["user_id"] == user.id]
        preferred_genres = self._get_preferred_genres(user)

        profile_vector = None
        if scenario != SCENARIO_NEW_USER and not user_ratings.empty:
            valid_rows = user_ratings[user_ratings["movie_id"].isin(self.runtime.movie_index_map.keys())]
            if not valid_rows.empty:
                indices = [self.runtime.movie_index_map[int(mid)] for mid in valid_rows["movie_id"].tolist()]
        ...
        similarities = cosine_similarity(profile_vector, self.runtime.content_similarity_matrix).flatten()
        score_map = {
        ...
        return candidates.sort_values(["final_score", "avg_rating"], ascending=False).reset_index(drop=True)
    ...
    def _item_scores(self, user: User, scenario: str) -> pd.DataFrame:
        ...
        if scenario == SCENARIO_NEW_USER or self.runtime.user_item_matrix.empty or user.id not in self.runtime.user_item_matrix.index:
            candidates["item_score"] = 0.0
            return candidates
        ...
        rated_movie_ids = [int(mid) for mid in user_row.index.tolist() if int(mid) in self.runtime.item_similarity_df.columns]
        if not rated_movie_ids:
            candidates["item_score"] = 0.0
            return candidates

        rating_vector = user_row.loc[rated_movie_ids].astype(float)
        similarity_subset = self.runtime.item_similarity_df.loc[candidates["movie_id"].tolist(), rated_movie_ids]
    ...
    def _svd_scores(self, user: User, scenario: str) -> pd.DataFrame:
        ...
        if scenario == SCENARIO_NEW_USER or user.id not in self.runtime.svd_prediction_df.index:
            candidates["svd_score"] = 0.0
            return candidates

        prediction_row = self.runtime.svd_prediction_df.loc[user.id]
        score_map = {int(mid): float(prediction_row.get(mid, self.runtime.global_mean)) for mid in candidates["movie_id"].tolist()}
        candidates = candidates.copy()
        candidates["svd_score"] = candidates["movie_id"].map(score_map).fillna(self.runtime.global_mean)
    ...
    def _hybrid_scores(self, user: User, scenario: str, popularity_df: pd.DataFrame, content_df: pd.DataFrame, item_df: pd.DataFrame, svd_df: pd.DataFrame) -> pd.DataFrame:
        weights = self._get_hybrid_weights(user.id, scenario=scenario)
        ...
        return base.sort_values(["final_score", "rating_count", "avg_rating"], ascending=False).reset_index(drop=True)

    def _materialize_items(self, ranked_df: pd.DataFrame, model_key: str, user: User, scenario: str) -> list[dict[str, Any]]:
        ...
        for row in ranked_df.itertuples(index=False):
            ...
            explanation = self._build_explanation(model_key=model_key, user=user, movie_id=movie_id, scenario=scenario, ranked_row=row)
    ...
    def _build_explanation(self, model_key: str, user: User, movie_id: int, scenario: str, ranked_row: Any) -> str:
        movie_data = self.runtime.movie_lookup.get(movie_id, {})
        ...
        if model_key == MODEL_CONTENT:
            return self._content_explanation(user=user, movie_id=movie_id, scenario=scenario)
        if model_key == MODEL_ITEM:
            return self._item_explanation(user=user, movie_id=movie_id, scenario=scenario)
        if model_key == MODEL_SVD:
            return self._svd_explanation(user=user, movie_id=movie_id, scenario=scenario)
    ...
    def _content_explanation(self, user: User, movie_id: int, scenario: str) -> str:
        ...
        if scenario != SCENARIO_NEW_USER and not self.runtime.ratings_df.empty:
            user_rows = self.runtime.ratings_df[self.runtime.ratings_df["user_id"] == user.id].sort_values("rating", ascending=False)
            for seen_movie_id in user_rows["movie_id"].astype(int).tolist():
                ...
                if common:
                    seen_title = self.runtime.movie_lookup.get(seen_movie_id, {}).get("title", "oldingi film")
    ...
    def _item_explanation(self, user: User, movie_id: int, scenario: str) -> str:
        if scenario == SCENARIO_NEW_USER or self.runtime.item_similarity_df.empty or user.id not in self.runtime.user_item_matrix.index:
        ...
        for seen_movie_id in user_row.index.tolist():
            if int(seen_movie_id) not in self.runtime.item_similarity_df.columns:
                continue
            similarity = float(self.runtime.item_similarity_df.at[movie_id, int(seen_movie_id)])
            if similarity > best_similarity:
                best_similarity = similarity
                best_movie_id = int(seen_movie_id)

        if best_movie_id is not None:
            seen_title = self.runtime.movie_lookup.get(best_movie_id, {}).get("title", "oldingi film")
    ...
    def _svd_explanation(self, user: User, movie_id: int, scenario: str) -> str:
        if scenario == SCENARIO_NEW_USER or self.runtime.svd_prediction_df is None or user.id not in self.runtime.svd_prediction_df.index:
        ...
        if not user_rows.empty:
            ...
            top_seen_title = self.runtime.movie_lookup.get(top_seen_id, {}).get("title", "oldingi film")
